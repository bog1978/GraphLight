...
internal class GenericGraph<G, V, E> : IGraph<G, V, E>
    ...
    where G : notnull
{
    private readonly Dictionary<V, GenericVertex<V, E>> _map = new();
    private readonly List<IEdge<V, E>> _edges = [];
    ...
    public void InsertVertex(IEdge<V, E> edge, V vertexData, E edgeData)
    {
        if (!Edges.Contains(edge))
            throw new GraphException("Данное ребро не принадлежит графу");
    ...
    public void Revert(IEdge<V, E> edge)
    {
        ...
        if (e.IsRevert)
            throw new GraphException("Edge is already reverted.");
        var src = e.Src;
        var dst = e.Dst;
