...
namespace GraphLight.Layout
{
    ...
    {
        public static Point2D CenterPoint(this IVertexAttrs node)
        {
            return new Point2D(node.Left + node.Width / 2, node.Top + node.Height / 2);
        }

        public static Point2D CustomPoint(this IVertexAttrs node, double wK, double hK)
        {
            return new Point2D(node.Left + node.Width * wK, node.Top + node.Height * hK);
        }

        public static Point2D GetShapePort(this IVertexAttrs node, Point2D point)
        {
